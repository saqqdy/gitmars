...
program.name("gitm review").usage("[--state [state]] [--quiet]").description("review\u8FDC\u7A0B\u4EE3\u7801");
if (args.length > 0)
...
program.action(async (opt) => {
  ...
  const { token } = userInfoApi ? await getUserToken() : {};
...
${iid}\uFF1A\u4E00\u5171\u53D8\u52A8\u4E86${changes_count}\u4E2A\u6587\u4EF6`));
      ...
      !opt.quiet && sendGroupMessage(`${appName}\u9879\u76EE${source_branch}\u5408\u5E76\u5230${target_branch}\u8BF7\u6C42ID${iid}\u6709\u65B0\u8BC4\u8BBA\uFF1A${note}`);
