...
program.action(async (type, name, opt) => {
  ...
  if (allow.includes(type) && name) {
    ...
    let cmd = [];
    if (!getIsUpdatedInTime({ lastet: "7d", limit: 1e3, branch: base })) {
      sh.echo(yellow("\u68C0\u6D4B\u5230\u8BE5\u5206\u652F\u5DF2\u7ECF\u8D85\u8FC71\u5468\u6CA1\u6709\u540C\u6B65\u8FC7\u4E3B\u5E72\u4EE3\u7801\u4E86\uFF0C\u8BF7\u6BCF\u5468\u81F3\u5C11\u540C\u6B65\u4E00\u6B21\uFF0C\u6267\u884C\uFF1Agitm update"));
    }
    if (opt.add) {
    ...
    if (opt.prod) {
      if (!opt.dev && !getIsMergedTargetBranch(`${type}/${name}`, config.develop, true)) {
        sh.echo(yellow(`\u68C0\u6D4B\u5230\u4F60\u7684\u5206\u652F\u6CA1\u6709\u5408\u5E76\u8FC7${config.develop}\uFF0C\u8BF7\u5148\u5408\u5E76\u5230${config.develop}\u5206\u652F`));
        process.exit(1);
      } else {
