...
const config = getConfig();
commander.program.name('gitm combine').usage('[type] [name] [-d --dev] [-p --prod]').description('合并bugfix任务分支、合并feature功能开发分支、合并support分支');
...
commander.program.action(async (type, name, opt) => {
  ...
  if (!opt.dev && !opt.prod) {
    sh__default['default'].echo('请输入需要同步到的环境');
    sh__default['default'].exit(1);
  }
  ...
  if (opt.commit === true) {
    sh__default['default'].echo(error(`请输入要提交的message`));
    sh__default['default'].exit(1);
  }
  ...
  if (allow.includes(type) && name) {
    ...
    if (opt.dev) {
      cmd = cmd.concat([`git fetch`, `git checkout ${config.develop}`, `git pull`, {
    ...
    if (opt.prod) {
      ...
      if (type === 'bugfix' && opt.asFeature) {
        cmd = cmd.concat(!level || level < 3 ? [`git fetch`, `git checkout ${config.release}`, `git pull`, {
          cmd: `git merge --no-ff ${type}/${name}`,
          config: {
            slient: false,
            again: false,
