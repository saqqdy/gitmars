...
program.action(async (branches, opt) => {
  ...
  if (opt.list) {
    if (_willDeleteBranch.length > 0) {
      ...
      const prompt = {
        type: "checkbox",
        message: yellow(`\u627E\u5230${_willDeleteBranch.length}\u6761\u5206\u652F\u5408\u5E76\u8FC7${targets.join(",")}\u5206\u652F\uFF0C\u8BF7\u9009\u62E9\u8981\u6E05\u7406\u7684\u5206\u652F`),
        name: "selectBranches",
        choices: []
      };
      ...
      inquirer.prompt(prompt).then(({ selectBranches }) => {
        if (selectBranches.length === 0) {
          echo(yellow("\u6CA1\u6709\u9009\u62E9\u4EFB\u4F55\u5206\u652F\uFF0C\u8FDB\u7A0B\u5DF2\u9000\u51FA"));
          process.exit(0);
        }
        selectBranches.forEach(async (branch) => {
          await clean(branch);
        });
      });
    } else {
    ...
    echo(green("\u5220\u9664\u5B8C\u6210\uFF0C\u8FD9\u4E9B\u5206\u652F\u5DF2\u88AB\u6E05\u7406\uFF1A" + _willDeleteBranch.join(" ")));
  }
});
