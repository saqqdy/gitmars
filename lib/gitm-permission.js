...
program.name('gitm permission').usage('[message]').arguments('[message]').description('校验提交权限').option('--no-verify', '是否需要跳过校验权限', false).action(function () {
        ...
        switch (_context.prev = _context.next) {
          ...
          case 2:
            ...
            if (current === config.master && !opt.noVerify && msg && msg.indexOf('Merge:') === -1 && msg.indexOf('Merge branch') === -1) {
              sh.echo(error("".concat(config.master, "\u5206\u652F\u4E0D\u5141\u8BB8\u76F4\u63A5\u63D0\u4EA4")));
              sh.exit(1);
            } else {
              sh.exit(0);
            }
