...
const createProgram = program.name("gitm admin").usage("<command> <type>").description("\u521B\u5EFAbugfix\u3001release\u3001develop\u548Csupport\u5206\u652F").command("create " + createArgs(create.args));
create.options.forEach((o) => {
  createProgram.option(o.flags, o.description, o.defaultValue);
});
createProgram.action((type) => {
  ...
  if (opts.includes(type)) {
    ...
    queue(cmd).then((data) => {
      if (data[3].status === 0) {
        echo(`${config[type]}\u5206\u652F\u521B\u5EFA\u6210\u529F\uFF0C\u8BE5\u5206\u652F\u57FA\u4E8E${base}\u521B\u5EFA\uFF0C\u60A8\u5F53\u524D\u5DF2\u7ECF\u5207\u6362\u5230${config[type]}
\u9700\u8981\u53D1\u7248\u65F6\uFF0C\u8BB0\u5F97\u6267\u884C: ${green("gitm admin publish " + config[type])}`);
      }
    });
  } else {
...
const publishProgram = program.name("gitm admin").usage("<command> <type> [--description [description]] [-c --combine] [--use-rebase] [-p --prod] [-b --build [app]] [--postmsg]").description("\u53D1\u5E03bugfix\u3001release\u3001support\u5206\u652F").command("publish " + createArgs(publish.args));
publish.options.forEach((o) => {
  publishProgram.option(o.flags, o.description, o.defaultValue);
});
publishProgram.action(async (type, opt) => {
  const {
    token,
    level,
    nickname = ""
...
const updateProgram = program.name("gitm admin").usage("<command> <type> [-m --mode [mode]] [--description [description]] [--use-rebase] [--postmsg]").description("\u66F4\u65B0bugfix\u3001release\u3001support\u5206\u652F\u4EE3\u7801").command("update " + createArgs(update.args));
update.options.forEach((o) => {
  updateProgram.option(o.flags, o.description, o.defaultValue);
});
updateProgram.action(async (type, opt) => {
  const {
    token,
    level,
    nickname = ""
...
const cleanProgram = program.name("gitm admin").usage("<command> <type>").description("\u6784\u5EFA\u6E05\u7406\u5DE5\u4F5C").command("clean " + createArgs(clean.args));
clean.options.forEach((o) => {
  cleanProgram.option(o.flags, o.description, o.defaultValue);
});
cleanProgram.action((type) => {
...
const approveProgram = program.name("gitm admin").usage("[type] [-k --key [keyword]]").description("\u6279\u51C6\u5408\u5E76\u8BF7\u6C42").command("approve " + createArgs(approve.args));
approve.options.forEach((o) => {
  approveProgram.option(o.flags, o.description, o.defaultValue);
});
approveProgram.action(() => {
