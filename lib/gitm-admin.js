...
if (publish.args.length > 0) {
  const _program = program.name("gitm admin").usage("<command> <type> [-c --combine] [--use-rebase] [-p --prod] [-b --build [app]] [--postmsg]").description("\u53D1\u5E03bugfix\u3001release\u3001support\u5206\u652F").command("publish " + createArgs(publish.args));
...
if (update.args.length > 0) {
  const _program = program.name("gitm admin").usage("<command> <type> [-m --mode [mode]] [--use-rebase] [--postmsg]").description("\u66F4\u65B0bugfix\u3001release\u3001support\u5206\u652F\u4EE3\u7801").command("update " + createArgs(update.args));
