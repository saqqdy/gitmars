...
program.action(async (command, args2, opt) => {
  ...
  if (command === "init") {
    ...
    console.log(types, process.env, process.argv, getBranchesFromID("2080d17e"));
    if (current !== config.develop && mainBranches.includes(current) && types.includes("1")) {
      ...
      if (command2 === "merge") {
        const isMergedBranch = getIsMergedTargetBranch(branch, config.develop, true);
        if (!isMergedBranch) {
          console.info(red("\u68C0\u6D4B\u5230\u4F60\u7684\u5206\u652F\u6CA1\u6709\u5408\u5E76\u8FC7" + config.develop));
          process.exit(0);
        } else {
    ...
    if (mainBranches.includes(current) && types.includes("2")) {
      ...
      if (command2 === "merge") {
        ...
        if (!isUpdatedInTime) {
          console.info(red("\u68C0\u6D4B\u5230\u4F601\u5468\u5185\u6CA1\u6709\u540C\u6B65\u8FC7\u4E3B\u5E72" + branchPrefix + "\u5206\u652F\u4EE3\u7801"));
          process.exit(0);
        } else {
