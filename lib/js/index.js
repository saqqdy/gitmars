...
function getLogs(config = {}) {
  ...
  const results = sh.exec(`git log${limit ? ' -"' + limit + '"' : ""}${lastet ? ' --since="' + getSeconds(lastet) + '"' : ""}${branches ? ' --branches="*' + branches + '"' : ""} --date-order --pretty=format:"${keys.join(",=")}-end-"`, { silent: true }).stdout.replace(/[\r\n]+/g, "").replace(/-end-$/, "");
  const logList = [];
