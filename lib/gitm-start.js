...
const config = getConfig();
commander.program.name('gitm start').usage('<type> <name>').description('创建bugfix任务分支、创建feature功能开发分支、support框架支持分支');
...
commander.program.action((type, name) => {
  ...
  if (opts.includes(type)) {
    ...
    queue(cmd).then(data => {
      if (data[3].code === 0) {
        sh__default['default'].echo(`${name}分支创建成功，该分支基于${base}创建，您当前已经切换到${type}/${name}\n如果需要提测，请执行${success('gitm combine ' + type + ' ' + name)}\n开发完成后，记得执行: ${success('gitm end ' + type + ' ' + name)}`);
      }
    });
  } else {
    sh__default['default'].echo(error('type只允许输入：' + JSON.stringify(opts)));
    sh__default['default'].exit(1);
  }
});
