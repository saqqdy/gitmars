					...
					<div class="section">
						...
						>
							...
							<v3-collapse-item name="9">
								<template #title>
									<MapCommand
										:value="commandValue['admin']['publish']"
										:current="data.current"
										execName="gitm admin"
										@exec="exec"
									></MapCommand>
								</template>
								<Command v-model="commandValue['admin']['publish']"></Command>
							</v3-collapse-item>
						</v3-collapse>
					</div>
					<div class="section">
						...
						>
							...
							<v3-collapse-item name="15">
								...
								<Command v-model="commandValue['cleanbranch']"></Command>
							</v3-collapse-item>
							...
							<v3-collapse-item name="18">
								...
								<Command v-model="commandValue['clean']"></Command>
							</v3-collapse-item>
...
<script lang="ts">
import { computed, defineComponent, inject, onErrorCaptured, onMounted, reactive } from 'vue'
import { onBeforeRouteLeave, useRoute, useRouter } from 'vue-router'
import Command from './comp/command.vue'
import MapCommand from './comp/map-command.vue'
import * as commandSets from 'gitmars/lib/conf/index'
...
export default defineComponent({
	...
	async setup() {
		...
		// @ts-expect-error
		const commandValue: Record<string, CommandSetsType> | any = reactive(commandSets)
		...
		const createBranch = () => {
			$box(boxAddBranchVue, {
				...
				onOk: async (instance: any) => {
					const { type, name } = await instance.component.proxy.submit()
