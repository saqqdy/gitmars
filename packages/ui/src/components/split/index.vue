	...
	setup(props, { slots, emit }) {
		...
		const getTransform = el => {
			const transformMatrix =
				...
				getComputedStyle(el, '').getPropertyValue('-webkit-transform') ||
				el.style.transform ||
		...
		const mousedown = e => {
			...
			data.boxSize = parseInt(
				props.mode === 'horizontal' ? v3Split.value.clientWidth : v3Split.value.clientHeight
			)
			while (el.parentNode) {
		...
		onMounted(() => {
			...
			if (data.suffix !== '%') {
				...
				data.size = calculateSize(
					fixNumber((parseInt(props.modelValue) * 100) / data.boxSize, 2)
				)
			}
		})
			...
			h(
				...
				slots.default
					...
					: [
							...
							((props.mode === 'horizontal' && slots.left && slots.right) ||
								(props.mode === 'vertical' && slots.top && slots.bottom)) &&
								h(
									'div',
									{
