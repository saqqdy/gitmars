...
<script>
import { computed, createVNode, nextTick, reactive, ref, render, unref, watchEffect } from 'vue'
...
export default {
	...
	setup(props) {
		...
		function calculate() {
			const c = 110 - (props.showBtn ? 0 : 60) - (props.showHeader ? 0 : 50) // herder=50和footer=60高度
			const p = props.showHeader ? 0 : 40
			const maxW = fixNumber(window.innerWidth > 320 ? window.innerWidth - 20 : 300)
			const maxH = fixNumber(
				window.innerHeight > 300 + c ? window.innerHeight - c - 60 + p : 240 + p
			)
			const maxHeight = parseInt(props.maxHeight || props.height || maxH || 480)
			const maxWidth = parseInt(props.maxWidth || props.width || maxW || 600)
		...
		function handleMax() {
			const c = 110 - (props.showBtn ? 0 : 60) - (props.showHeader ? 0 : 50) // herder=50和footer=60高度
			const p = props.showHeader ? 0 : 40
			const w = fixNumber(window.innerWidth > 320 ? window.innerWidth - 20 : 300)
			const h = fixNumber(
				window.innerHeight > 300 + c ? window.innerHeight - c - 60 + p : 240 + p
			)
			if (!data.max) {
