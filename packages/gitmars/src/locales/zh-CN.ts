export default {
    ...
    'Execute method': '执行方法',
    'Do not push the message': '不要推送消息',
    'Query branch for keywords': '查询分支的关键词',
    ...
    'Delete branch': '删除分支',
    'Force branch deletion': '强行删除分支',
    'Set association with remote branches': '设置与远程分支关联',
    ...
    'Execute commit, information required': '执行commit，需填写信息',
    'Description of the reason for this commit': '本次提交的原因描述',
    'Execute add': '执行add',
    ...
    AccessKeyword: '存取关键字',
    'Serial Number': '序号',
    'Keep staging area not deleted': '保留暂存区不删除',
    'Command Name': '指令名称',
    'Do you want to skip the check permission': '是否需要跳过校验权限',
    'Query logs after a certain time, fill in the format: 10s/2m/2h/3d/4M/5y':
        '查询在某个时间之后的日志，填写格式：10s/2m/2h/3d/4M/5y',
    'The maximum number of logs to be queried': '最多查询的日志条数',
    'Detection type': '检测类型',
    'Branch to query': '要查询的分支',
    'Version number': '版本号',
    'Plugin Name': '插件名称',
    'Whether to use Taobao Mirror': '是否使用淘宝镜像',
    ...
    'Update all local bugfix, feature, support branches':
        '更新本地所有bugfix、feature、support分支',
    'The name of the client used to load the package': '用于装包的客户端名称',
    ...
    'There are no files staged in this branch!': '该分支没有暂存任何文件！',
    'File recovery successful': '文件恢复成功',
    ...
    'Log query': '日志查询',
    'Merge branch code': '合并分支代码',
    'Verify commit permissions': '校验提交权限',
    'Undo a commit record': '撤销一次提交记录',
    'Please select the undo record to restore': '请选择要恢复的撤销记录',
    'Please select the merge request to be operated': '请选择要操作的合并请求',
    'Comment List': '评论列表',
    Comments: '评论',
    'File staging successful': '文件暂存成功',
    ...
    'Branch name does not conform to specification': '分支名称不符合规范',
    ...
    'Gitmars can only be executed in a git environment, so please install git first':
        'Gitmars只能在git环境下执行，请先安装git',
    'View gitmars version number': '查看gitmars版本',
    ...
    'Gitmars does not provide the command "gitm {command}", it has been passed through to git for execution, here are the results':
        'Gitmars没有提供“gitm {command}”这个指令，已透传到git执行，下面是执行结果：',
    'Send group message': '发送群消息',
    'Create bugfix, release, develop and support branches':
        '创建bugfix、release、develop和support分支',
    ...
    'Cannot be deleted: {something}': '不可删除：{something}',
    'Analysis completed: {something}': '分析完成：{something}',
    'Undo successfully: {something}': '撤销成功：{something}',
    ...
    'The record {id} is detected as a revert record, please use the "gitm redo" operation':
        '检测到{id}这条记录是一条revert记录，请使用"gitm redo"操作',
