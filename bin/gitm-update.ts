...
program.action((type: string | string[], name: string, opt: GitmBuildOption) => {
    ...
    branchList.forEach((branch: string) => {
        ...
        const base: string = type === 'bugfix' ? config.bugfix : type === 'support' ? config.master : config.release
        const cmd: Array<CommandType | string> = ['git fetch', `git checkout ${base}`, 'git pull', `git checkout ${type}/${name}`]
        if (opt.useRebase) {
            cmd.push({
                cmd: `git rebase ${base}`,
